...
dat <- future_map(path_files, read_info)
...
dat %>%
  group_by(maf.cat) %>%
  summarise(n(), mean(Rsq, na.rm = TRUE), sd(Rsq, na.rm = TRUE)) %>%
  rename("MAF Category" = maf.cat, n = "n()",
         "mean rsq" = "mean(Rsq, na.rm = T)",
         "sd rsq" = "sd(Rsq, na.rm = T)") %>%
  mutate(n = format_number(n)) %>%
