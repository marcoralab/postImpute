...
no_rsq2 = is.na(snakemake@params[["rsq2"]]) || snakemake@params[["rsq2"]] == "NA"
...
samp_pretty <- prettyNum(snakemake@params[["sampsize"]], big.mark=",", preserve.width="none")
...
myFiles <- choose_files(snakemake@params[["path"]], chr_parse(snakemake@params[["chrom"]]))
...
myPathFiles <- paste0(snakemake@params[["path"]], myFiles)
...
dat.out <- dat %>%
  mutate(Pass_R2_Common = Rsq >= snakemake@params[["rsq"]] & MAF >= snakemake@params[["maf"]]) %>%
...
if ( no_rsq2 ) {
  ...
  dat.out %<>%
    mutate(Pass_R2_Rare = Rsq >= snakemake@params[["rsq2"]] & MAF < snakemake@params[["maf"]]) %>%
...
write_csv(dat.out, paste0(snakemake@params[["output_dir"]], '/SNPcount', cohort_file, '.csv'))
```
...
ggplot(data = dat, aes(x = Rsq, fill = CHR)) + geom_histogram(binwidth = 0.01) +
  ...
  labs(title = paste0(snakemake@params[["cohort"]], ': Distribution of imputation quality scores'), x = bquote('Quality ('~R^2~')'))
```
...
ggplot(dat, aes(x = maf.cat, y = Rsq, fill = maf.cat)) + theme_bw() +
  ...
  labs(title = paste0(snakemake@params[["cohort"]], ': Distribution of Quality score by MAF '),
  x = 'MAF',
...
## Plot Infoscore by genomic position
 ggplot(data = dat.smallsamp, aes(x = POS, y = Rsq)) + geom_point(size = 0.1, alpha = 0.5) +
  ...
  labs(title = paste0(snakemake@params[["cohort"]], ': Quality score by genomic position'),
  x = 'Genomic Position',
  y = bquote('Quality ('~R^2~')'))
```
...
ggplot() +
  ...
  geom_vline(xintercept = snakemake@params[["maf"]], colour = 'blue', linetype = 2) +
  geom_hline(yintercept = rsqs, colour = 'red', linetype = 2) +
  theme_bw() + labs(title = paste0(snakemake@params[["cohort"]], ': Quality score by MAF'), y = bquote('Quality ('~R^2~')')) +
