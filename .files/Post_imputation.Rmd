...
params:
   #path: /Users/sheaandrews/Desktop/F1/
   path: ./ADGC_ACT1-2/
   chrom: "20:22"
   cohort: Sample
   maf: 0.01 #cutoff between common and rare
   rsq: 0.3 #Rsq to use with common variants
   rsq2: NA #Rsq to use with rare variants (set to !r NA to use rsq as hard cutoff)
...
#library(knitr)     ## kniting document (and making tables)
...
##  Read in chromsome info files (chr*.info.gz) from HRC imputation, filtering by the chromosome list.
...
chrom <- chr_parse(params$chrom)

myFiles <- list.files(path = params$path, pattern = "*.info.gz")
files_use = c()
chroms_missing = c()
for (i in chrom){
  ch = paste0("chr",i,".info.gz")
  chrom_present = grepl(ch, myFiles)
...
##---- Data Wrangling ----##
##  empty list for reading files into
data.ls <- list()

##  Loop through chromsome files to read them into R
# replace '-' with NA
# seperate chr:pos into sepeate columns
# change pos and EmpRsq to numeric varibles
# create new varible with MAF bins
for(i in myFiles){
    dat.raw <- read_tsv(paste0(params$path,i), col_types = 'cccnnnncnnncc', na = c("-"))
    dat <- dat.raw %>%
      separate(SNP, c('CHR', 'POS'), sep = ":") %>%
      mutate(POS = as.integer(POS)) %>%
      unite(SNP, c(CHR, POS, 'REF(0)', 'ALT(1)'), sep = ':', remove = F) %>%
      mutate(maf.cat = cut(MAF, breaks=c(-Inf, 0.0005, 0.001, 0.005, 0.01, 0.02, 0.05, Inf),
                           labels=c('< 0.05%', '0.05% to 0.1%', '0.1% to 0.5%', '0.5% to 1%', '1% to 2%', '2% to 5%', '≥ 5%'))) %>%
      select(SNP, CHR, POS, MAF, Rsq, EmpRsq, Genotyped, maf.cat)
    data.ls[[i]] <- dat
    cat(paste0("Chr: ", i, "\n"))
  }

# bind data frame together and relevel CHR
dat <- bind_rows(data.ls)
...
##  Subsample either 50k imputed and genotyped SNPs OR if less then 50k SNPs avalibe, all SNPs
sampsize = params$sampsize
datImput.smallsamp <- dat %>%
  filter(Genotyped == 'Imputed') %>%
  group_by(CHR) %>%
  sample_frac(min(1, sampsize/imputed.snps), replace = F) %>% ## Don't use number greater than 1 if sample size is greater than actual number of SNPs.
  ungroup()
...
dat.out <- dat %>% mutate(cut.rsq.high = Rsq >= params$rsq & MAF >= params$maf) %>%
  mutate(cut.typed = Genotyped == "Genotyped")

if ( no_rsq2 ) {
  dat.out %<>%
    mutate(cut.maf = MAF >= params$maf) %>%
    mutate(included = (cut.rsq.high & cut.maf) | cut.typed)
} else {
  dat.out %<>%
    mutate(cut.rsq.low = Rsq >= params$rsq2 & MAF < params$maf) %>%
    mutate(included = cut.rsq.low | cut.rsq.high | cut.typed)
}
...
dat.out %<>%
  ...
  filter_all(all_vars(!is.na(.))) %>%
  select(CHR, matches("cut.rsq.low|cut.maf"), cut.rsq.high, cut.typed, included) %>%
  group_by(CHR) %>%
...
dat.out %<>%
  bind_rows(summarise_all(select(dat.out, -CHR), funs(mean))) %>%
  bind_rows(summarise_all(select(dat.out, -CHR), funs(sum)))
dat.out[nrow(dat.out), "CHR"] <- "Total"
dat.out[nrow(dat.out)-1, "CHR"] <- "Mean"

dat.out %<>%
  mutate(n = prettyNum(n, big.mark=",", preserve.width="none")) %>%
  mutate(cut.rsq.high = prettyNum(cut.rsq.high, big.mark=",", preserve.width="none")) %>%
  mutate(cut.typed = prettyNum(cut.typed, big.mark=",", preserve.width="none")) %>%
...
if ( no_rsq2 ) {
  dat.out %<>%
    rename(cut.rsq = cut.rsq.high) %>%
    mutate(cut.maf = prettyNum(cut.maf, big.mark=",", preserve.width="none"))
} else {
  dat.out %<>%
    mutate(cut.rsq.low = prettyNum(cut.rsq.low, big.mark=",", preserve.width="none"))
}

## save the table to disk
write_csv(dat.out, paste0(params$path, 'SNPcount', cohort_file, '.csv'))
```
...
fig <- plt %>% plot_ly(x = ~MAF, y = ~Quality, color = ~Type,
  type="scatter", mode="markers", customdata = c(~SNP, ~Type),
  ...
  config(sendData = FALSE, displaylogo = FALSE, collaborate = FALSE,
